...
infinity = float('inf')
GameState = namedtuple('GameState', 'to_move, pedras, pedras_restantes, ponta1, ponta2, moves, utility, num_pedras')
...
def expectiminimax(game,state):
    ...
    def chance_node(state, action, depth):
        ...
        for chance in chances:
            ...
            util = 0
            if res_state.to_move%2 == player%2:
...
class Domino(StochasticGame):
    ...
    def __init__(self):
        ...
        self.initial = GameState(to_move=0, pedras=list(self.jogadores[0]), pedras_restantes=pedras_restantes, ponta1=None,ponta2=None,
                                 moves=self.moves(0,self.jogadores[0], pedras_restantes, None, None, 0), utility=0, num_pedras=(7,7,7,7))
    ...
    def eval_amiga(self, state, player):
        ...
        m = len(state.pedras_restantes) - p / 2
        buchas = self.buchas(state.pedras)
        soma_pedras = self.soma_pedras_max(state)   
        delta = m - (n+p)
        return -0.9*buchas - 0.2*n + 0.7*delta - 0.4*soma_pedras - 0.2*p
    ...
    def eval(self, state, player):  			#Para testes
        ...
        if player == 0 or player == 2:
            m = (len(state.pedras_restantes) - p)/2
            delta = m - (n+p)
            return -0.9*buchas - 0.2*n_pedras + 0.7*delta - 0.3*soma_pedras - 0.2*p
        else:
            m = (len(state.pedras_restantes))/3
            delta = m - n
            return -0.9*buchas - 0.2*n_pedras + 0.7*delta - 0.3*soma_pedras
    ...
    def compute_utility(self, to_move, pedras, pedras_restantes, ponta1, ponta2, move,player, state):
        if move[0].valor[0] == -1 and ponta1 is not None:
            ...
            menor = 100 
            for i in range(4):
                if i != player and state.num_pedras[i] < menor:
                    menor = state.num_pedras[i]
            for i in range(menor):
                soma_res += pedras_restantes[i].getValor()
            if soma < soma_res:
                if to_move%2 != player%2:
                    return -1
                else:
                    return 4
            else:
            ...
            if to_move%2 == player%2:
                if len(pedras) == 1 or state.num_pedras[(player+2)%4] == 1:
                    return 4
                else:
                    return 0
            else:
                if state.num_pedras[to_move] == 1:
                    return -1
                else:
                    return 0
        else:
    ...
    def menor_soma(self):
        return argmax([0, 1],
               key=lambda a: -(self.soma_pedras(a)+self.soma_pedras(a+2)))
    ...
    def result(self, state, move, player):
        ...
        ponta1 = state.ponta1
        ponta2 = state.ponta2
        utility = self.compute_utility(state.to_move, state.pedras, state.pedras_restantes, state.ponta1,state.ponta2, move, player, state)
        num_pedras = list(state.num_pedras)
        if move[0].valor[0] != -1:
            if state.ponta1 is not None:
                if move[1] == 0:
            ...
            if state.to_move == player:
                ...
                num_pedras[player] = num_pedras[player] - 1
            else:
                pedras_restantes.remove(move[0])
                num_pedras[state.to_move] = num_pedras[state.to_move] - 1
        ...
        new_state = GameState(to_move=to_move, pedras=pedras, pedras_restantes=pedras_restantes, ponta1=ponta1,
                              ponta2=ponta2, moves=self.moves(to_move, pedras, pedras_restantes, ponta1, ponta2, player), 
                              utility=utility, num_pedras=num_pedras)
        return new_state
    ...
    def outcome(self, state, chance):
        return GameState(to_move=state.to_move, pedras=state.pedras, pedras_restantes=state.pedras_restantes, ponta1=state.ponta1,
                         ponta2=state.ponta2, moves=chance[1], utility=state.utility, num_pedras=state.num_pedras)
        ...
        while True:
            ...
            for player in players:
                ...
                if vencedor != -1:
                    ...
                    print(str(vencedor) + " e " + str(vencedor+2))
                    return vencedor
                ponta1 = state_res.ponta1
                ponta2 = state_res.ponta2
                to_move = state_res.to_move
                num_pedras= list(state_res.num_pedras)
                pedras_restantes = []
                for i in range(4):
                    if i != to_move:
                        pedras_restantes += self.jogadores[i]
                state = GameState(to_move=to_move, pedras=list(self.jogadores[to_move]), pedras_restantes=pedras_restantes, ponta1=ponta1, ponta2=ponta2,
                                  ...
                                  utility=self.compute_utility(to_move, self.jogadores[to_move], pedras_restantes, ponta1, ponta2, move,to_move, state), num_pedras=num_pedras)
...
if __name__ == "__main__":
    graph_winrates(5, expectiminimax, expectiminimax, expectiminimax, expectiminimax)
